...
frames_t _output_frames(frames_t avail, struct thread_ctx_s *ctx) {
	...
	if (ctx->config.send_silence && ctx->output.state == OUTPUT_START_AT) {
		// virtually sets "now" in the future to take into account latency
		u32_t now = gettime_ms() + (ctx->output.latency * 1000L) / ctx->output.current_sample_rate;
		...
		if (now >= ctx->output.start_at || ctx->output.start_at > now + 10000) {
			...
			frames = min(frames, MAX_SILENCE_FRAMES);
			LOG_ERROR("too early %u", ctx->output.start_at - now);
		}
	}
