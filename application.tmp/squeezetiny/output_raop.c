...
static void *output_raop_thread(struct thread_ctx_s *ctx) {
	while (ctx->output_running) {
		// proceed only if room in queue -or- wait for start
		if ((raopcl_avail_frames(ctx->output.device) >= FRAMES_PER_BLOCK) || ctx->output.state == OUTPUT_START_AT) {
			LOCK;
			...
			if (!ctx->config.send_silence && ctx->output.state == OUTPUT_START_AT) {
				u64_t latency = TS2NTP(raopcl_latency(ctx->output.device), ctx->output.current_sample_rate);

				if (raopcl_set_start(ctx->output.device, TIME_MS2NTP(ctx->output.start_at) - latency)) {
					ctx->output.state = OUTPUT_RUNNING;
					LOG_INFO("[%p]: starting at %u (n:%u)", ctx, ctx->output.start_at, gettime_ms());
				}
			}

			// only send silence when this is for skipping
			if (ctx->config.send_silence || (ctx->output.state >= OUTPUT_RUNNING && ctx->output.state != OUTPUT_START_AT)) {
				u8_t *buffer;
				int size;
				u64_t playtime;
		...
		ctx->output.device_frames = raopcl_queued_frames(ctx->output.device);
		if (!ctx->config.send_silence) ctx->output.device_frames += raopcl_latency(ctx->output.device);;
...
void output_raop_thread_init(struct raopcl_s *raopcl, unsigned output_buf_size, struct thread_ctx_s *ctx) {
	...
	ctx->output.latency = ctx->config.send_silence ? raopcl_queue_len(raopcl) : raopcl_latency(raopcl);
