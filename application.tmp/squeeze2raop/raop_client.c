...
/*
 ...
 --- queuing & pause ---
  ...
  expressed in frames format, for simplicity
  Say that latency is 10 frames, the queue size is 10 frames and we want to
  start at t=2000, assuming now=1000
  Say that the player will want to pause at t=2200
  "avail()" means calls to raopcl_avail_frames() and we assume that buffer fill
  takes no time
  ...
  - When a call to raopcl_send_chunk is made, head_ts=first_ts=1990
  - At t=1500 avail is 100 as cons=0, total=1990-1990=0
  - Immediately after that, buffer is filled so when head_ts=2090, avail turns
	to zero and no further frames can be filled
  - At this point, the client has provided 100 frames the head_ts has been
	incremented by 100 (is now 2090)
  - At t=2000 cons=2000-1990=10, total=2090-1990=100, avail is 10 so the client
	sends another 10, so the total of supplied frames is 110 just after t=2000
	and head_ts is now 2100.
  - To calculate the number of frames already *heard*, the client must use what
	he has sent (110), minus what is in the buffer (100), minus the latency (10)
	so at t=2000, 0 frames have been heard!
  - At t=2001, cons=2001-1990=11, total=2100-1990=110, so avail=100-(110-11)=1
	frame can be filled, so just after that, the client has sent 111 frames and
	played 111-100-10 = 1 frame. The value of head_ts is 2101
  - At t=2200, right after filling frames, cons=2200-1990=210,
	total=2300-1990=310, so avail=100-(310-210)=0 and the client has provided
  ...
  - At this point, the client decides to stop and call raopcl_set_pause(). That
	sets pause_ts at head_ts, so 2300. Client must absolutely *not* call
	raopcl_send_chunk() and must call raoplc_flush() to mute audio
  - Any further call to avail will use pause_ts, so avail will remains at 0 as
	long as nothing else happens. When calculating what has been heard, client
	does 310(sent)-100(queue)-10(latency)=200 which is correct
  - The client at t=2400, the client decides to resume at 2500, so it expects
	that the 1st frame to be played, will be at 2500 precisely and that frame
	will the 201 frames BECAUSE	ONLY 200 AS BEEN PLAYED SO FAR
  - As it should, the client calls raopcl_set_start with 2500-10=2490. That sets
	first_ts=start_ts=2490 and starts checking again for room in the queue, but
	because we are still in pause mode, avail calculates from pause_ts, so it
	continues to report the same as above, so the client is stuck and cannot
	call raopcl_send_chunk whose job is to set all xxxx_ts properly.
	//TODO: this has to change
  - So here there is a small trick where, when paused, avail returns an extra
	chunk of availability (note that raopcl_queued_frames() does *not* increase by that extra chunk)
  - The client calls raopcl_send_chunk with its next available frame, but this
	is frame 311, which would be sent with a timestamp of 2490, so it would be
	heard at 2500, instead of 2611. In other words, a pause offsets	playback by
	queue_len + latency frames.
  (let's forget the +1 frame question for the moment)
  - This would be detectable by the client because at t=2500, cons=2500-2490=10,
	total=2490-2490=0, so the client would fill quickly 90 frames, so finally
	at t=2500, it would see 310+90=400(sent)-100(queue)-10(latency)=390 frames
	sent where in fact 200 should have been, so the time gap could be detected
	and corrected, but this is not a desirable solution
  -
...
/*----------------------------------------------------------------------------*/
__u64 raopcl_elapsed_frames(struct raopcl_s *p)
{
	__u64 now_ts, elapsed;

	if (!p || p->state < RAOP_FLUSHED) return 0;
	...
	elapsed = (now_ts > p->first_ts) ? now_ts - p->first_ts : 0;
...
/*----------------------------------------------------------------------------*/
__u64 raopcl_elapsed_time(struct raopcl_s *p)
{
	return TS2NTP(raopcl_elapsed_frames(p), p->sample_rate);
}
...
__u32 raopcl_avail_frames(struct raopcl_s *p)
{
	...
	// at least or frame has been transmitted or we are in pause
	if (p->first_ts || p->pause_ts) {
		__u64 now_ts;
		__u64 total, consumed;
		...
		if (consumed > total) avail = p->queue_len;
		else if (total > consumed + p->queue_len) avail = 0;
			else avail = p->queue_len - (total - consumed);
	}
	// not paused, not transmitted a first frame ==> queue is empty
	else avail = p->queue_len;
...
/*----------------------------------------------------------------------------*/
void _raopcl_set_timestamps(struct raopcl_s *p, new_ts)
	// can reset head_ts if desired now_ts is above it
	if (new_ts > p->head_ts) {
		p->head_ts = new_ts;
		p->offset_ts = 0;
	}
	else p->offset_ts = p->head_ts - new_ts;
...
/*----------------------------------------------------------------------------*/
bool raopcl_start(struct raopcl_s *p, __u64 start_time)
{
	__u64 now_ts, start_ts;

	if (!p) return false;

	pthread_mutex_lock(&p->mutex);

	// wait till flush is done
	if (p->state != RAOP_FLUSHED) {
		LOG_WARN("[%p]: waiting for flush", p);
		pthread_cond_wait(&p->flush_cond, &p->mutex);
	}

	// not paused and not specific start time ==> start with 1st sent frame
	if (!start_time && !p->pause_ts) {
		LOG_INFO("[%p]: will start with first frame", p);
		p->first_ts = 0;
		pthread_mutex_unlock(&p->mutex);
		return true;
	}
	...
	start_ts = NTP2TS(start_time, p->sample_rate);
	first_ts = start_ts ? start_ts : now_ts;

	LOG_INFO("[%p]: start time %Lu (ts:%Lu)", p, start_time, p->first_ts);

	// normal start, maybe at a given time
	if (!p->pause_ts) _rtp_set_timestamps(p, first_ts);
	// un-pausing, maybe with a start time
	else {
		struct sockaddr_in addr;
		rtp_audio_pkt_t *packet;
		__u32 n, i;
		__u32 len = (p->head_ts - p->offset_ts - p->pause_ts);

		LOG_INFO("[%p]: queue before %u (hts: %Lu n:%Lu ofs:%u)", p, len, p->head_ts, p->first_ts, p->offset_ts);
		if (p->first_ts + len > p->head_ts + p->offset_ts) p->offset_ts = 0;
		p->head_ts = p->first_ts + len + p->offset_ts;

		LOG_INFO("[%p]: queue after %u (hts: %Lu ofs:%u)", p, len, p->head_ts, p->offset_ts);
...
bool raopcl_send_chunk(struct raopcl_s *p, __u8 *sample, int size, __u64 *playtime)
{
	...
	// wait till flush is done if start or pause has been required
	if (p->state != RAOP_FLUSHED && (p->start_ts || p->pause_ts)) {
		LOG_WARN("[%p]: waiting for flush", p);
		pthread_cond_wait(&p->flush_cond, &p->mutex);
		LOG_WARN("[%p]: waiting for flush done", p);
	}

	now = get_ntp(NULL);
	now_ts = NTP2TS(now, p->sample_rate);

	// this was a start without pause or a start time, need to set timestaps
	if (!p->first_ts) _rtp_set_head(now
	...
	*playtime = TS2NTP(p->head_ts - p->offset_ts + raopcl_latency(p), p->sample_rate);
	...
	LOG_SDEBUG("[%p]: sending audio ts:%Lu (pt:%u.%u now:%Lu) ", p, p->head_ts, SEC(*playtime), FRAC(*playtime), now);
	...
	if (FD_ISSET(p->rtp_ports.audio.fd, &wfds)) {
		...
		usleep((p->chunk_len * 1000000LL) / (p->sample_rate * 2));
	}
	else {
