...
sub binaries {
	...
	if ($os->{'os'} eq 'Linux') {
		...
		if ($os->{'binArch'} =~ /arm/) {
			return qw(squeeze2raop-armv6hf squeeze2raop-armv5hf squeeze2raop-armv5hf-static);
		}
		
		# fallback to offering all linux options for case when architecture detection does not work
		return qw(squeeze2raop-x86-64 squeeze2raop-x86-64-static squeeze2raop-x86 squeeze2raop-x86-static squeeze2raop-armv6hf squeeze2raop-armv5hf squeeze2raop-armv5hf-static);
	}
